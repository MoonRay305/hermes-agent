---
name: spec
description: "Use when the operator says `/spec <idea>` or asks to spec out an idea. Runs a structured interview plus read-only repository and Linear reconnaissance, then creates one verified, bridge-compatible Linear build packet only after explicit approval."
version: 2.0.0
...
metadata:
  hermes:
    ...
    related_skills: [linear, github-pr-workflow]
---
